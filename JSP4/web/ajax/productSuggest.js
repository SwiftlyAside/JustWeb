...
function findFunc() {
  ...
  xhr.onreadystatechange = () => {
    if (xhr.status === 200 && xhr.readyState === 4) {
      ...
      if (!(data === '' || data == null || (typeof data === 'object' && !Object.keys(data).length))) { sel.style.display = 'block'; } else { sel.style.display = 'none'; }

      // 응답된 JSON 타입 데이터를 select 상자에 표시
      for (const datum of data) { sel.options.add(new Option(datum.name, datum.id)); }
    }
  };
  xhr.send();
}
